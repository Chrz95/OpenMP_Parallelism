...
char* reverseConstString(char const* str)
{
 ...
 int Smith_Waterman (char* FirstString,char* SecondString,char* output_filename,int match,int mismatch,int gap,FILE **fp,int pair)
 {
		...
		for (int i = 1 ; i < length2 + 1  ; i++)
		{
				for (int j = 1 ; j < length1 + 1; j++)
				{
							...
							if ((ScoringMatrix[index] > MaxValue) && (ScoringMatrix[index] !=0)) // If it finds a new max value
							{
								cnt = 0;
							...
							else if (ScoringMatrix[index] == MaxValue) // If it finds the same max value
							{
									cnt++ ;
									if (cnt < length2 + 1)
									{
	...
	traceback_length = length2/100 + 2 ;
 ...
 void Read_Dataset (char* output_filename,char* path,int match,int mismatch,int gap,int NumOfThreads)
 {
	...
	#pragma omp parallel num_threads(NeededThreads) shared(fp,fp1,pair,Q_max,D_Sz_all,Traceback_Steps,pairs) private(i,ch,Q_string,D_string)
	{
			#pragma omp for 
			for (i=0 ; i< pairs ; i++) 
			{
				///////////////// Read a pair of strings /////////////////////////////////////
				#pragma omp critical // Only one thread must read a Q,D pair each time
				{	
					...
					//printf ("My Thread_Id is %d and my pair is %d \n",omp_get_thread_num(),pair);
				...
				Traceback_Steps += Smith_Waterman (Q_string,D_string,output_filename,match,mismatch,gap,&fp1,pair);
